...
- **`translated`** — `true` when the English on the page is a translator's rather than the author's, which is the case for 17 of the current 30 entries. It makes the prompt tell that member to take the cadence and shape of the argument but not the specific diction, so Teresa doesn't inherit E. Allison Peers's vocabulary as her own. Read the citation rather than grepping it for "trans.": Moina Mathers's 1926 preface is her own English, and the "trans." in its citation refers to S.L. MacGregor Mathers translating the book she was prefacing.
...
Coverage as an exemplar is therefore counted by distinct `author`, not by union of `members`: 30 of 38 roster members, not the 31 that `members` gives. (The roster grew from 33 to 38 in the 2026-08-10 TV-personas addition — see the 2026-08-10 #35a entry in STATUS.md for the five new members this increment covered.)

**After adding an entry, run `node scripts/count-library-coverage.js` and update the two hand-written numbers above (and the `translated` count) to match its output**, rather than hand-computing the delta from memory. Two concurrent #35a PRs (Pauli [#318](https://github.com/msdixon/secret-cabinet/pull/318), Yates [#322](https://github.com/msdixon/secret-cabinet/pull/322)) each bumped these same lines from the same stale base on 2026-08-19 — git's merge didn't even conflict on the text, it just silently left the wrong count in place until someone recomputed it by hand. `test/library.test.js` now asserts these lines match the script's output, so a stale number fails `npm test` instead of drifting until the next doc-checkin catches it.
