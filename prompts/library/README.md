...
- **`translated`** — `true` when the English on the page is a translator's rather than the author's, which is the case for 19 of the current 33 entries. It makes the prompt tell that member to take the cadence and shape of the argument but not the specific diction, so Teresa doesn't inherit E. Allison Peers's vocabulary as her own. Read the citation rather than grepping it for "trans.": Moina Mathers's 1926 preface is her own English, and the "trans." in its citation refers to S.L. MacGregor Mathers translating the book she was prefacing.
