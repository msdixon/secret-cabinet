...
It checks every entry's `source_url` against its host: archive.org entries via the `/metadata/<id>` API (which reliably distinguishes a real item from an invented one — the `/details/<id>` page alone does not), `doi.org` links via Crossref, and `books.google.com`/`google.com/books/edition/...` links via the Google Books API's per-volume lookup — all for the same reason: a plain HTTP check can pass for a plausible-but-wrong identifier, not just a fabricated one. Google Books closes the gap the other two leave for commercially published, in-copyright, non-journal 20th/21st-century books, which have neither an archive.org identifier nor a DOI. Anything else falls back to a plain HTTP-200 check, which is weaker — worth keeping in mind when choosing a `source_url` for a non-public-domain entry. Non-zero exit if anything's broken.
...
- **`translated`** — `true` when the English on the page is a translator's rather than the author's, which is the case for 25 of the current 44 entries. It makes the prompt tell that member to take the cadence and shape of the argument but not the specific diction, so Teresa doesn't inherit E. Allison Peers's vocabulary as her own. Read the citation rather than grepping it for "trans.": Moina Mathers's 1926 preface is her own English, and the "trans." in its citation refers to S.L. MacGregor Mathers translating the book she was prefacing.
...
Coverage as an exemplar is therefore counted by distinct `author`, not by union of `members`: 38 of 38 roster members, as of the 2026-08-21 Corbin/Sun Ra entries closing #316's gap-filling round. (The roster grew from 33 to 38 in the 2026-08-10 TV-personas addition — see the 2026-08-10 #35a entry in STATUS.md for the five new members this increment covered.)

### A second entry per author — [#370](https://github.com/msdixon/secret-cabinet/issues/370) wave 2

Once every member had one authored entry, [#370](https://github.com/msdixon/secret-cabinet/issues/370) started adding a *second*, deliberately different-genre one for some members — a letter, a diary, an epistle, alongside the original formal preface/treatise — to widen the register #187's exemplar shows. `test/library.test.js` no longer forbids an author appearing more than once (it now only guards against a third entry sliding in unreviewed); `loadVoiceExemplar` still takes the *first* library.json match per author as the primary exemplar, and a new `loadSecondaryVoiceExemplars` reads any later ones. The speaker prompt renders the primary passage at full weight and appends secondary entries as smaller, explicitly-supplementary "tone-tuning" text — evidence that sharpens or corrects the read the primary gives, not a second competing exemplar. See `src/library.js` and `src/pipeline-speaker.js`'s `buildVoiceExemplarSection` for the implementation, and order a member's entries in library.json deliberately: whichever you want as primary goes first.
...
`source_url` for a non-public-domain entry should point to something a reader can independently verify the license or provenance from — a DOI, a publisher/journal page, a Google Books/Google Scholar or institutional-repository link — not just a PDF with no bibliographic trail. `scripts/verify-library-sources.js` checks `doi.org` links against Crossref and `books.google.com` links against the Google Books API the same way it checks archive.org identifiers against `/metadata`, so a fabricated DOI or Google Books id fails the same way a fabricated archive.org id does. A WorldCat-style catalog check would round this out further but isn't wired in yet — OCLC's Search API needs an institutional key this project doesn't have.
