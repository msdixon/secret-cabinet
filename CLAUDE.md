...
**The primary checkout itself (`/Users/racheldixon/Documents/GitHub/secret-cabinet`, no `.claude/worktrees/` suffix) is not a worktree and must always be left on `main` when a session is done with it.** It's the reference point `/cabinet-next`'s Step 1 and the weekly doc-checkin verify against, and the place those same commands push small docs-only commits directly to `main` from. Real feature or content work — even something that feels like "just one more quick commit," like curating one more library entry — belongs in its own worktree under `.claude/worktrees/`, same as everything else. Checking out a feature branch in the primary checkout and leaving it there blocks every other session that expects to find `main` there.

**Why:** on 2026-08-18, a `/cabinet-next` session ("Cabinet Next & library additions") did its docs work correctly in the primary checkout, but then also used that same directory for a second, unrelated library-curation commit (`#35: add Adorno library entry`) instead of spinning up a worktree — and ended with the checkout still parked on that feature branch, unpushed. A later `/cabinet-next` session found the checkout on `main` at the start (Step 1's check passed) but by the time it went to push its own approved PROJECT.md edit, the checkout had been switched away by the still-unfinished session's branch — the docs commit landed on the wrong branch and had to be extracted and reapplied from an isolated worktree instead. Nothing was lost, but it was a near miss: a `git reset --hard` next to another session's uncommitted work.
...
- **Before creating a new worktree, if `.claude/worktrees/` already has more than a few entries, do a quick pass first**: for each, check whether its branch is merged and whether it has uncommitted changes (`git status --short`).
  - Merged + clean → remove it outright, **unless it has zero commits since branching from `main`** (`git log main..<branch> --oneline` empty). A zero-commit worktree is trivially "merged" (its tip is a no-op ancestor of `main`) and trivially "clean" (nothing written yet) whether it's genuinely abandoned or a session that's mid-investigation and simply hasn't committed yet — nothing in git state distinguishes the two. Skip it; an empty worktree costs nothing to leave alone. On 2026-08-21, a `/cabinet-next` session's hygiene pass swept a running chip session's own worktree this way, mid-turn, on exactly these two signals — no data was lost only because that session hadn't written anything yet (see `STATUS.md`, filed as [#392](https://github.com/msdixon/secret-cabinet/issues/392)).
...
**A single shared keyword does not fan out across a comma-separated list of issue numbers.** `Closes #507, #508, #509, #510` looks bare-and-adjacent for every number in it, but GitHub's parser only honored the first — #507 auto-closed on merge, #508/#509/#510 did not, confirmed 2026-09-02 by checking issue state after PR #522 merged. **When one PR closes multiple issues, repeat the keyword for each:** `Closes #507, closes #508, closes #509, closes #510` (or one keyword per line). This is a different failure mode from the adjacency rule above — the keyword was adjacent to #507, just not to the rest of the list — so don't assume the single-issue guidance already covers it.
