...
**Refinement (2026-08-20) — this applies to what occupies the frame, not only to which controls exist.** Every one of the four precedents below *removed machinery*; none of them asked what should be on screen at rest. That left a gap the principle as originally written didn't catch: the `/cabinet-review` pass on 2026-08-20 measured the app's own front page at 2124px tall with the 3D room beginning at y=1339 — nearly two viewport heights below the fold, 341px tall, and never co-visible with anything else. Nothing there is a control panel by the original definition. There are no stray toggles and no cold-open grid. But the resting state of the page is still a form with the room filed underneath it, which is the same failure the principle exists to prevent, expressed as layout rather than as controls. Rachel, confirming the widening at that review: the principle *"needs a more universal application."* So: **the diegetic default governs spatial hierarchy and attention too — what the user is looking at before they act, not just what they can click.** Applied in [#358](https://github.com/msdixon/secret-cabinet/issues/358) (the room moves above the controls, controls arrange around it) and, notably, in that issue's own gating condition: a diegetic resting state has to actually be worth resting on, which is why [#357](https://github.com/msdixon/secret-cabinet/issues/357) (the room has no fireplace, despite the runtime prose invoking the fire ~14 times) blocks it rather than following it. Choosing the diegetic shape obliges you to make the fiction good enough to carry the frame.
