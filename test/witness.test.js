...
test('stage view switcher (#202): text presentation ⇄ room, on the stage container', async t => {
  ...
  await t.test('setStageView("room") switches the stage pane and button state, not the record', t2 => {
    ...
    assert.equal(
      document.getElementById('stage-record').classList.contains('view-room'),
      false,
      'view-room belongs to the stage pane, not the stage/record assembly'
    );
  });
  ...
  await t.test('ignores an unrecognized view rather than clearing the current one', t2 => {
    ...
    assert.ok(
      document.getElementById('stage-pane').classList.contains('view-room'),
      'an invalid call should be a no-op, not fall back to text'
    );
  });
