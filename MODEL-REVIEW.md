...
1. **Cost per session.** Pull `generationMetrics` from a few recent real sessions (`sessions/*.json` — `.generationMetrics[]`, has `phase`, `usage.input_tokens`/`usage.output_tokens`, `skipped`). Multiply by current per-token pricing for the model under review. Compare against the current default at the same call volume.
   - **Known gap, tracked as [#225](https://github.com/msdixon/secret-cabinet/issues/225):** only `director`/`speaker`/`disposition` phases (inside `runRound`) get pushed to `generationMetrics`. `/api/cast` and `/api/sessions/:id/verify-citations` call `MODEL` directly but never log usage anywhere persisted — there's no real cost data for casting or citation verification. If #225 is still open when you run a review, either pick it up first or explicitly flag the resulting cost estimate as director+speaker-only, not the full picture.
