...
1. **Cost per session.** Pull `generationMetrics` from a few recent real sessions (`sessions/*.json` — `.generationMetrics[]`, has `phase`, `usage.input_tokens`/`usage.output_tokens`, `skipped`). Multiply by current per-token pricing for the model under review. Compare against the current default at the same call volume.
   - **Known gap, worth closing before this is trustworthy:** only `director`/`speaker`/`disposition` phases (inside `runRound`) get pushed to `generationMetrics`. `/api/cast` and `/api/sessions/:id/verify-citations` call `MODEL` directly but never log usage anywhere persisted — there's no real cost data for casting or citation verification. If this is still true when you run a review, either fix it first (route those calls' `onMetric`/response `.usage` into something persisted) or explicitly flag the resulting cost estimate as director+speaker-only, not the full picture.
